...
require 'csv'
CSV.open("restaurantFinal.csv","w",:write_headers=> true,:headers=>["Item_name","qty"]) do |csv|
...
csvObj = CSV.read("restaurantFinal.csv")   #csvObj is array as object
...
module Food
    ...
    def Food::select_items (h={})
          ...
          if (qty>$csv_hash[item_name].to_i)
             ...
             CSV.open("restaurantFinal.csv", "wb") {|csv| $csv_hash.to_a.each {|elem| csv << elem} }
...
module For_File
    def For_File::set_details(t,p)
        file = File.new("RestaurantBillFinal.txt", "w")
        file.syswrite("##############################################################\n")
